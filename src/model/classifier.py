...
class classifier_hateClipper_(nn.Module):
    def __init__(self, image_dim, text_dim, num_layers, proj_dim, map_dim, fusion_mode, dropout=None, batch_norm=False, dataset=None) -> None:
        super(classifier_hateClipper_, self).__init__()
        self.fusion_mode = fusion_mode
        ...
        self.mlp = nn.Sequential(*layers)
        if dataset == 'FB' or dataset == 'HarMeme' or dataset == 'MMHS' or dataset == 'Tamil' or dataset == 'HarmP':
            self.output_layer = nn.Linear(proj_dim, 1)
